...
void Analysis::charge_sharing(std::string cfg, int ch1,int ch2,int ch3=-1)
{
	...
	if (ch3 == -1){
		...
		plotter.Plot1D(Form("chargesharing_threehits_%i_%i_%i_xpos"      ,ch1,ch2,ch3),     ";x [mm]"        , x_dut[dut], 100, min_X, max_X);
		...
		plotter.Plot1D(Form("chargesharing_twohits_%i_%i_%i_sum_amp"  		,ch1,ch2,ch3),         ";Sum Amplitude [mV]"            , sum_charge , 50, 0, 3000);
		plotter.Plot2D(Form("chargesharing_twohits_%i_%i_%i_sharing_%i_%i"  ,ch1,ch2,ch3,ch1,ch2), ";Amplitude [mV]; Amplitude [mV]", amp[ch1], amp[ch2] , 50, 0, 1800, 50, 0, 1800);
		plotter.Plot2D(Form("chargesharing_twohits_%i_%i_%i_sharing_%i_%i"  ,ch1,ch2,ch3,ch1,ch3), ";Amplitude [mV]; Amplitude [mV]", amp[ch1], amp[ch3] , 50, 0, 1800, 50, 0, 1800);
		plotter.Plot2D(Form("chargesharing_twohits_%i_%i_%i_sharing_%i_%i"  ,ch1,ch2,ch3,ch2,ch3), ";Amplitude [mV]; Amplitude [mV]", amp[ch2], amp[ch3] , 50, 0, 1800, 50, 0, 1800);
